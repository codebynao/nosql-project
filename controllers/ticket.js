...
class Ticket {
   ...
   */
	async upsertTicket(request) {
		try {
			...
			console.error('error upsertTicket', err) // eslint-disable-line no-console
   ...
   */
	async deleteTicket(request) {
		try {
			...
			console.error('error getTicketDetails', err) // eslint-disable-line no-console
			return { code: 50000, data: null, error: err.message }
		}
	}
   ...
   */
	async getAllTickets() {
		try {
			...
			console.error('error getAllTickets', err) // eslint-disable-line no-console
			return { code: 50000, data: null, error: err.message }
		}
	}
   ...
   */
	async getUserTickets(request) {
		try {
			...
			console.error('error getUserTickets', err) // eslint-disable-line no-console
			return { code: 50000, data: null, error: err.message }
		}
	}
   ...
   */
	async getTicketDetails(request) {
		try {
			const ticketId = request.params.ticketId
			if (!ticketId) {
				throw new Error('No ticket id in request')
