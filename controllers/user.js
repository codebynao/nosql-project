   ...
   */
	async upsertUser(request) {
		try {
			...
			console.error('error upsertUser', err) // eslint-disable-line no-console
   ...
   */
	async deactivateUser(request) {
		try {
			...
			console.error('error deactivateUser', err) // eslint-disable-line no-console
			return { code: 50000, data: null, error: err.message }
		}
	}
   ...
   */
	async getAllUsers() {
		try {
			...
			console.error('error getAllUsers', err) // eslint-disable-line no-console
			return { code: 50000, data: null, error: err.message }
		}
	}
   ...
   */
	async getUserDetails(request) {
		try {
			...
			console.error('error getUserDetails', err) // eslint-disable-line no-console
			return { code: 50000, data: null, error: err.message }
		}
	}
}
